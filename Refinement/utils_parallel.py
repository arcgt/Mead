...
def mouth_center(mouth_landmark):
    center_x = torch.tensor((mouth_landmark[:, 1] + mouth_landmark[:, 13])/2, dtype = torch.int16)
    center_y = torch.tensor((mouth_landmark[:, 0] + mouth_landmark[:, 12])/2, dtype = torch.int16)
    return center_x, center_y

def mouth_extract(base_img, center_x, center_y, mask_width, mask_height):
    ...
    for i in range(base_img.shape[0]):
        ...
        base = -1 * np.ones((3, 384, 384), dtype=np.uint8)
        mask0 = np.zeros((3, mask_height, mask_width), dtype=np.uint8)
        base[:, start_y[i]:start_y[i]+mask_height, start_x[i]:start_x[i]+mask_width] = mask0

        base_mask = np.zeros((3, 384, 384), dtype=np.uint8)
        mask1 = np.ones((3, mask_height, mask_width), dtype=np.uint8)
        base_mask[:, start_y[i]:start_y[i]+mask_height, start_x[i]:start_x[i]+mask_width] = mask1

        mouth = base_img *torch.tensor(base_mask, dtype = torch.float32).cuda()
        mouth = mouth + torch.tensor(base, dtype = torch.float32).cuda()
        mouth_list.append(mouth)
    mouth_batch = mouth_list[0]
    for j in range(1, base_img.shape[0]):
        mouth_batch = torch.cat([mouth_batch, mouth_list[j]], dim = 0)

    return mouth_batch

def mouth_crop(base_img, center_x, center_y, mask_width, mask_height):
    start_x = center_x - int(mask_width/2)
    start_y = center_y - int(mask_height/4)
    mouth_list = []
    #print(center_x, center_y, start_x, start_y)
    for i in range(base_img.shape[0]):

        base = np.zeros((3, 384, 384), dtype=np.uint8)
        mask0 = -1 * np.ones((3, mask_height, mask_width), dtype=np.uint8)
        base[:, start_y[i]:start_y[i]+mask_height, start_x[i]:start_x[i]+mask_width] = mask0

        base_mask = np.ones((3, 384, 384), dtype=np.uint8)
        mask1 = np.zeros((3, mask_height, mask_width), dtype=np.uint8)
        base_mask[:, start_y[i]:start_y[i]+mask_height, start_x[i]:start_x[i]+mask_width] = mask1
